...
function main(){
  ...
  function normalizeTweetTextSpacing(text){
    // Normalize strange whitespace so the Notes composer doesn't treat long ranges as a single @mention
    text = text
  ...
  function isAdTweet(article){
    try{
      // Look for explicit Ad/Promoted labels or aria-labels
      const nodes = article.querySelectorAll('span, div, a, svg[aria-label]');
      for (const el of nodes){
        const t = (el.textContent || '').trim().toLowerCase();
        if (t === 'ad' || t === 'promoted') return true;
        const aria = (el.getAttribute('aria-label') || '').trim().toLowerCase();
        if (aria === 'ad' || aria === 'promoted') return true;
      }
      // Some ads include special data-testid markers
      if (article.querySelector('[data-testid="placementTracking"], [data-testid*="promoted" i]')) return true;
    }catch{}
    return false;
  }
  ...
  async function openComposeWith(text) {
    ...
    if (text.length <= 1800) {
      ...
      params.set('note_token', token); // Keep token for tracking
      params.set(NOTE_PREFILL_PARAM, text); // 'message' parameter with full text
      
      // Also store in storage as backup
      try {
        const { pendingNotes } = await chrome.storage.local.get(['pendingNotes']);
        const map = pendingNotes && typeof pendingNotes === 'object' ? pendingNotes : {};
        map[token] = { text: String(text).slice(0, 6000), ts: Date.now() };
        await chrome.storage.local.set({ pendingNotes: map, pendingNoteToken: token });
      } catch {
        await chrome.storage.local.set({ 
          pendingNoteText: String(text).slice(0, 6000), 
          pendingNoteTs: Date.now(), 
          pendingNoteToken: token 
        });
      }
  ...
  function injectButton(container){
    ...
    if (isAdTweet(container)) return; // never inject into ads
