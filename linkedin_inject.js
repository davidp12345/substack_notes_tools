...
function main(){
  ...
  function isValidMediaUrl(urlStr){
    try{
      ...
      // Accept media hosts, including experimental buckets like media-exp1.licdn.com
      if (/^static\.licdn\.com$/.test(host)) return false;
      if (!/^media(?:-exp\d+)?\.licdn\.com$/.test(host)) return false;
      const path = u.pathname || '';
      // Must be a DMS image path; v1 or v2 both occur
      if (!/\/dms\/image(\/v2)?\//i.test(path)) return false;
      // Exclude obvious non-content assets
      if (/articleshare|author|logo|company-logo_|profile-displayphoto|emoji|reaction|reactions|like|love|icons?/i.test(path)) return false;
      // Previously we required "feedshare"; however many valid posts use "image-shrink_*" or other variants
      // e.g., .../image-shrink_800/... So we no longer enforce a specific variant here
      return true;
    }catch{ return false; }
  }
  ...
  function findImageLinks(container) {
    const srcs = [];
    // Include <source> elements (used inside <picture>) and various lazy-load attributes
    const selectors = 'img, source[srcset], [data-delayed-url], [data-src], [data-delayed-background-image], [data-background-image-url], [style*="background-image"]';
    container.querySelectorAll(selectors).forEach(el => {
      ...
      const delayedBg = el.getAttribute('data-delayed-background-image'); if (delayedBg) candidates.push(delayedBg);
      const bgUrl = el.getAttribute('data-background-image-url'); if (bgUrl) candidates.push(bgUrl);
  ...
  function injectProfileGridOverlays(){
    // Skip overlays entirely on profile pages to avoid confusing UX where clicks open the post
  ...
  // Inject overlays only where appropriate (non-profile contexts)
